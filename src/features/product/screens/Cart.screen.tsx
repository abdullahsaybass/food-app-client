...
//                             {deliveryCharge > 0 ? `MVR ${deliveryCharge} Delivery` : 'Free Delivery'}
//                           </Text>
...
//                       : <Text style={[s.billingValue, { color: deliveryCharge > 0 ? '#EF4444' : PRIMARY }]}>
//                           {deliveryCharge > 0 ? `MVR ${deliveryCharge}` : 'FREE'}
//                         </Text>
//                     }
//                   </View>
//                   <View style={s.billingRow}>
...
export const CartScreen: React.FC = () => {
  ...
  const cartLoading    = useProductStore(s => s.cartLoading);
  const cartCount      = useProductStore(s => s.cartCount);
  const cartError      = useProductStore(s => s.cartError);       // FIX: read cartError
  const updateQty      = useProductStore(s => s.updateQuantity);
  const removeFromCart = useProductStore(s => s.removeFromCart);
  const addToCart      = useProductStore(s => s.addToCart);
  const isLoggedIn     = useAuthStore(s => !!s.token);
  ...
  const [loadingCharge, setLoadingCharge]         = useState(false);
  const [deliveryZoneLabel, setDeliveryZoneLabel] = useState<string | null>(null);
  // FIX: track zone fetch error so we can warn the user instead of silently showing "Free"
  ...
  useEffect(() => {
    ...
    API.post('/delivery-zones/resolve', {
      ...
      // FIX: was silently falling back to 0 (showing "Free Delivery") on error.
      // Now we set zoneError=true so the UI can warn the user instead.
      .catch(() => {
        setDeliveryCharge(0);
        setDeliveryZoneLabel(null);
        setZoneError(true);
  ...
  return (
    <View style={s.safe}>
      ...
      {/* FIX: show cartError when cart mutations (add/remove/update) fail */}
      {cartError ? (
                  ...
                  <View style={s.freeDeliveryLeft}>
                    ...
                    <View style={{ flexShrink: 1 }}>
                      {loadingCharge ? (
                        ...
                        <>
                          <Text style={s.freeDeliveryTitle}>
                            {/* FIX: show warning title if zone lookup failed */}
                            {zoneError
                          ...
                          <Text style={s.freeDeliverySub}>
                            {/* FIX: honest message when zone API fails */}
                            {zoneError
                              ? 'Could not fetch delivery charge. Final cost shown at checkout.'
                              : deliveryZoneLabel
                                ? `Delivering to ${deliveryZoneLabel}`
                                : deliveryCharge > 0
                                  ? 'Delivery charge applies to your area'
                ...
                <View style={s.billingCard}>
                  ...
                  <View style={s.billingRow}>
                    ...
                    {loadingCharge
                      ? <ActivityIndicator size="small" color={PRIMARY} />
                      : zoneError
                        /* FIX: show "—" with a note instead of the misleading "FREE" */
                        ? <Text style={[s.billingValue, { color: TEXT2 }]}>—</Text>
                        : <Text style={[s.billingValue, { color: deliveryCharge > 0 ? '#EF4444' : PRIMARY }]}>
                            {deliveryCharge > 0 ? `MVR ${deliveryCharge}` : 'FREE'}
                          </Text>
                    }
                  ...
                  <View style={s.billingRow}>
                    <Text style={s.billingTotalLabel}>To Pay</Text>
                    <Text style={s.billingTotalValue}>
                      {zoneError ? `${formatPrice(subtotal)} + delivery` : formatPrice(totalAmount)}
                    </Text>
                  </View>
                </View>
...
const s = StyleSheet.create({
  ...
  // FIX: cart mutation error banner (add/remove/update failures)
  cartErrorBanner: {
